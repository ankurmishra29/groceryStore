...
const RecommendedProduct = ({ categoryName }) => {
  ...
  const filteredProducts = categoryName
    ? allProduct.filter((product) => product.category === categoryName)
    : allProduct

  return (
    <div
      className={`relative max-w-[85rem] px-4 py-8 sm:px-6 lg:px-4 lg:py-4 mx-auto bg-gray-300 ${
        themeHeader === "fixed" ? "top-10 lg:top-8 mb-10 lg:mb-4" : ""
      }`}
    >
      <div className="py-2 px-6 flex justify-between items-center">
        <h2 className="font-bold text-xl text-slate-700 capitalize">
          {categoryName ? `Recommended Product` : "All Products"}
        </h2>
      </div>
